...
- (void)deserializeJSONStringFromFile:(NSString*)filePath;
...
@synthesize backgroundProgressViewController;
@synthesize dropboxSyncDelegate;
...
- (void)makeAlert {
    ...
    //add a background process and reset it's message

    self.backgroundProgressViewController = [self.dropboxSyncDelegate backgroundProgressViewController];
    [self.dropboxSyncDelegate showBackgroundProgress];
    self.backgroundProgressViewController.percentageLabel.text = @"";
...
- (void)alertMessageAppendPercentage:(float)percent {
    if (percent<1) { // Don't report the 100% because that looks silly
        ...
        self.backgroundProgressViewController.percentageLabel.text = $str(@"%.0f%%",percent*100.0);
    }else {
        self.backgroundProgressViewController.percentageLabel.text = $str(@"0%%");
    }
}
...
- (void)alertCompleteWithMessage:(NSString*)message {
    ...
    [self.dropboxSyncDelegate hideBackgroundProgress];
...
- (void)tellDelegateComplete {
    ...
    if ([self.dropboxSyncDelegate respondsToSelector:@selector(syncComplete)]) {
        // Do it after a short delay so the caller (dbrestclient) gets a chance to finish what it's doing before we zombify it!
        [self.dropboxSyncDelegate performSelector:@selector(syncComplete) withObject:nil afterDelay:0.001];
    }
}
...
- (void)getLocalStatusForFiles:(NSDictionary**)_files andFolders:(NSDictionary**)_folders {
    ...
    NSString* dataFiles = [root stringByAppendingPathComponent:kLocalDataFolder];
    NSMutableSet* pathsToSearch = $mset(dataFiles);
...
- (void)restClient:(DBRestClient*)client loadMetadataFailedWithError:(NSError*)error {
    [self failure:[NSString stringWithFormat:@"Couldn't get file list from Dropbox: %@", [error localizedDescription] ]];
}
...
//    for(NSString* filename in localFiles.allKeys)
//    {
//        NSLog(@"local file: %@",filename);
//    }
//    
//    for(NSString* filename in remoteFiles.allKeys)
//    {
...
#pragma mark - Main SYNC flow of control
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex == alertView.firstOtherButtonIndex) {
        ...
        //        //sync capable of running in background
        //        isSynchronizing = YES;
        //        
...
#pragma mark -
#pragma mark WEB JSON TO CORE DATA

-(void)handleDownloadRequestInBackgroundState:(NSURLRequest*)urlRequest
{
//    {
//        
//        NSString* trimmedString  =[urlRequest.URL.absoluteString stringByReplacingOccurrencesOfString:@"https://api-content.dropbox.com/1" withString: @""];
//        
//        RKObjectManager* objectManager = [RKObjectManager sharedManager];
//        
//        
//        ASIHTTPObjectLoader* objectLoader = [ASIHTTPObjectLoader loaderWithResourcePath:trimmedString objectManager:objectManager delegate:self];
//        objectLoader.objectMapping = [objectManager.mappingProvider objectMappingForClass:[AppUser class]];
//        objectLoader.shouldContinueWhenAppEntersBackground = YES;
//        
//        
...
//        [[RKParserRegistry sharedRegistry] setParserClass:parser forMIMEType:@"text/plain; charset=ascii"];
//        
//        //try again
//        if(![objectLoader canParseMIMEType:@"text/plain; charset=ascii"])
//        {
//            NSAssert(false,@"Parser not registered properly!");
//        }
//        
//        
//        RKCatalogAppDelegate* appDelegate = (RKCatalogAppDelegate*)[UIApplication sharedApplication].delegate;
//        [appDelegate enqueueRequest:objectLoader];
//        
//    }  
}
-(void)handleDownloadFileURLRequest:(NSURLRequest*)urlRequest
{
    
    //    [self handleDownloadRequestInBackgroundState:urlRequest];
    //    return;
    //    
    //    
    //
    ...
    //    
    //    
    //    [objectManager loadObjectsAtResourcePath:trimmedString delegate:self block:^(RKObjectLoader* loader) {
    //        loader.objectMapping = [objectManager.mappingProvider objectMappingForClass:[AppUser class]];
    //        
    //    }];
...
- (void)doTodoItem {
    ...
    if (remaining == 1) {
        ...
        self.backgroundProgressViewController.statusLabel.text = $str(@"%d tasks", remaining);;
    } 
...
#warning need to handle JSON deletion here
//            //DELETING LOCAL CORE DATA ENTITIES WHEN LOCAL DELETE HAPPENS

            if ([self.dropboxSyncDelegate respondsToSelector:@selector(deletedFileAtPath:)]) {
                [((id)self.dropboxSyncDelegate) performSelector:@selector(deletedFileAtPath:) withObject:localPath];
...
- (void)restClient:(DBRestClient *)client loadedFile:(NSString *)destPath 
{
    ...
    //convert back to core data
//    DLog(@"Downloaded a file!: %@", destPath);
    
    
    //deserialize JSON to core data
    NSString* pathExtension = [destPath pathExtension];
    if([pathExtension isEqualToString:@"json"] )
    {
        DLog(@"Downloaded a JSON FILE: %@", [destPath lastPathComponent]);
        if ([self.dropboxSyncDelegate respondsToSelector:@selector(deserializeFileAtPath:)]) {
            [((id)self.dropboxSyncDelegate) performSelector:@selector(deserializeFileAtPath:) withObject:destPath];
